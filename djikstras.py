        ...
        while True:
            ...
            for neighbor in self.d_table:
                if neighbor is not node:
                    ...
                    if (not self.geofence.intersect(node, neighbor)) and (self.d_table[neighbor].shortest_node is None or dist < self.d_table[neighbor].shortest_dist):
                        self.d_table[neighbor] = Djikstras.NodeInfo(dist, node)
            nodes.remove(node)
            valid_nodes = list(filter(lambda n: self.d_table[n].shortest_dist is not None, nodes))
